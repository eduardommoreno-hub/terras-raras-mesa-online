...
async function loadHub(){
  ...
  if(charsEl){
    ...
    charsEl.innerHTML = charsCache.map(c=>{
      ...
      const hubCard=`assets/characters/hub_cards/${id}_card_hub.webp?v=19.6.11.16`;
      return `<div class="char premiumCharacterCard hubOfficialCharacterCard" style="--char-accent:${esc(c.color||'#d0a94a')}" title="${esc(c.name)}">
        <img class="hubOfficialCharacterImage" src="${hubCard}" alt="${esc(c.name)}" onerror="this.onerror=null;this.src='${esc(c.card_url||'')}?v=19.6.11.16';">
      </div>`;
    }).join('');
  }
...
function renderVisualRightPanel(){
  ...
  const portraitHTML=ch?.card_url?`<button class="vtcPortrait cardAvatar" onclick="openMyCharacterModal()" title="Ver ficha"><img src="${esc(ch.card_url)}?v=19.6.11.16" alt="${esc(ch.name||name)}"></button>`:`<button class="vtcPortrait spriteAvatar" style="--sprite-pos:${sprite}" onclick="openMyCharacterModal()" title="Ver ficha"></button>`;
...
function trRefreshCharacterPreviewV1964(){
  ...
  if(ch?.card_url){ wrap.innerHTML=`<img src="${esc(ch.card_url)}?v=19.6.11.16" alt="${esc(ch.name)}"><div><b>${esc(ch.name)}</b><span>${esc(ch.role||'Personagem')}<br>${esc(ch.zone||ch.description||'Totem da aventura')}</span></div>`; }
...
function trCharacterFullCardImgHTMLV1969(c){
  ...
  return `<div class="characterFullCardFrame">${img ? `<img class="characterFullCardImg" src="${esc(img)}?v=19.6.11.16" alt="${esc(c?.title||'Carta de personagem')}" onerror="this.parentElement.innerHTML='<div class=&quot;wideCardMissing&quot;>✦</div>'">` : '<div class="wideCardMissing">✦</div>'}</div>`;
}
