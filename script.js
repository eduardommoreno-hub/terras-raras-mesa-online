...
function renderVisualRightPanel(){
  ...
  const portraitHTML=ch?.card_url?`<button class="vtcPortrait cardAvatar" onclick="openMyCharacterModal()" title="Ver ficha"><img src="${esc(ch.card_url)}?v=19.6.11.18" alt="${esc(ch.name||name)}"></button>`:`<button class="vtcPortrait spriteAvatar" style="--sprite-pos:${sprite}" onclick="openMyCharacterModal()" title="Ver ficha"></button>`;
...
function aiMode(){ return qs('aiMode')?.value || 'normal'; }
...
function trRefreshCharacterPreviewV1964(){
  ...
  if(ch?.card_url){ wrap.innerHTML=`<img src="${esc(ch.card_url)}?v=19.6.11.18" alt="${esc(ch.name)}"><div><b>${esc(ch.name)}</b><span>${esc(ch.role||'Personagem')}<br>${esc(ch.zone||ch.description||'Totem da aventura')}</span></div>`; }
