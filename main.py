...
Crie um resumo de sessão organizado em tópicos: eventos importantes, posição dos personagens, itens/fraquezas descobertas e ganchos para a próxima sessão."""
    return base + """
Gere a próxima narração da cena. Termine com 2 ou 3 opções de ação para as jogadoras."""
...
@app.get("/health")
def health(): return {"status":"ok", "service": APP_NAME, "version":"v6-local-ai-zero-cost"}
...
async def complete_ai_job(job_id: int, req: AICompleteReq, token: str = "", db: Session = Depends(db_dep)):
    ...
    if job.status == "done" and job.result:
        if job.job_type == "narrative":
            db.add(ChatMessage(room_id=job.room_id, user_id=job.user_id, text="🎙️ Narradora Local:\n" + job.result))
        elif job.job_type == "image_prompt":
            db.add(SessionNote(room_id=job.room_id, title="Prompt de imagem local", text=job.result))
        elif job.job_type == "summary":
            db.add(SessionNote(room_id=job.room_id, title="Resumo gerado pela IA local", text=job.result))
